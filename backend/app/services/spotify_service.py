...
class SpotifyServiceClient:
            ...
            try:
                ...
                if "401" in str(e) or "403" in str(e):
                    if attempt < max_retries:
                        logger.warning(f"Auth error on attempt {attempt + 1}, forcing refresh and retrying...")
        ...
        try:
            ...
            if "401" in error_str:
                ...
                if "user may not be registered" in error_str.lower():
                    logger.error(f"Token lacks required scopes or app configuration issue (HTTP 403): {e}")
                    logger.error("This usually means the refresh token was generated without proper scopes.")
    ...
    async def _refresh_client(self) -> None:
        ...
        for attempt in range(max_retries):
            try:
                logger.info(f"Refreshing service account Spotify client (attempt {attempt + 1}/{max_retries})")
                ...
                logger.info(f"Service account client refreshed for user: {user_info.get('display_name')} ({user_info.get('id')})")
                return  # Success, exit retry loop
                
            except Exception as e:
                is_last_attempt = attempt == max_retries - 1
                if is_last_attempt:
                    logger.error(f"Failed to refresh service account client after {max_retries} attempts: {e}")
                    ...
                    logger.warning(f"Refresh attempt {attempt + 1} failed: {e}. Retrying in {delay}s...")
    ...
    async def _validate_token_scopes(self, token_scope: str) -> None:
        ...
        if missing_scopes:
            logger.error(f"Access token is missing required scopes: {', '.join(missing_scopes)}")
            logger.error(f"Granted scopes: {', '.join(granted_scopes)}")
            logger.error(f"Required scopes: {', '.join(REQUIRED_SCOPES)}")
            logger.error("Please regenerate your refresh token with all required scopes.")
