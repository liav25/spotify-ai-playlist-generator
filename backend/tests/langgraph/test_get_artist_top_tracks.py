...
import pytest
from app.langgraph.tools import get_artist_top_tracks


class TestGetArtistTopTracks:
    """Test suite for get_artist_top_tracks tool"""

    def test_get_artist_top_tracks_successful(self, config_with_spotify_client, mock_spotify_client):
        ...
        result = get_artist_top_tracks.invoke(
            {"artist_id": artist_id, "country": country}, 
        ...
        mock_spotify_client.artist_top_tracks.assert_called_once_with(artist_id, country=country)

    def test_get_artist_top_tracks_default_country(self, config_with_spotify_client, mock_spotify_client):
        """Test get_artist_top_tracks with default country"""
        # Arrange
        artist_id = "test_artist_456"
        
        # Act
        result = get_artist_top_tracks.invoke({"artist_id": artist_id}, config_with_spotify_client)
        
        # Assert
        assert isinstance(result, list)
        mock_spotify_client.artist_top_tracks.assert_called_once_with(artist_id, country="US")
    ...
    def test_get_artist_top_tracks_api_error(self, config_with_spotify_client, mock_spotify_client):
    ...
    def test_get_artist_top_tracks_different_countries(self, config_with_spotify_client, mock_spotify_client):
